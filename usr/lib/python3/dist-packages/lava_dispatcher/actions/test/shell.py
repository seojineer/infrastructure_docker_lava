...
class TestShellAction(TestAction):
    ...
    def run(self, connection, max_end_time):  # pylint: disable=too-many-locals
        ...
        self.logger.debug("[SEOJI] connection_namespace:" + connection_namespace)
        self.logger.debug("[SEOJI] check yaml param self.parameters: " + str(self.parameters))
        parameters = None
        ...
        self.logger.debug("[SEOJI] send check_char "  + str(connection.check_char))
        ...
        if lava_test_results_dir is None:
            # Nexell extension
            if 'nexell_ext' in self.parameters:
                self.logger.debug("[SEOJI] nexell_ext exist! - set lava_test_results_dir.")
        ...
        try:
            ...
            for feedback_ns in self.data.keys():  # pylint: disable=no-member
                ...
                self.logger.debug("[SEOJI] self.parameters.get('namespace'):" + str(self.parameters.get('namespace')))
                ...
                if feedback_connection:
                    self.logger.debug("[SEOJI] feedback_connection: " + str(feedback_connection))
            ...
            with connection.test_connection() as test_connection:
                ...
                # Nexell extension
                self.logger.debug("[SEOJI] params: " + str(self.parameters.get('params')))
                definition_list = list(self.parameters['definitions'])
                self.logger.debug("[SEOJI] definition_list: " + str(definition_list))
                ...
                for item in definition_list:
                    ...
                    if 'params' in item:
                        self.logger.debug("[SEOJI] yes 'params' exist in item!")
                        if 'ROOT' in item['params']:
                            self.logger.debug("[SEOJI] yes 'ROOT' exist in item!")
                ...
                if root_cmd == True:
                    ...
                    self.logger.debug("[SEOJI] sendline 'sudo %s/bin/lava-test-runner %s/%s'" % (lava_test_results_dir, lava_test_results_dir, running))
                    ...
                    test_connection.sendline(self.parameters['nexell_ext']['password'])

                    self.logger.debug("[SEOJI] self.parameters: " + str(self.parameters))
                    if 'nexell_ext' in self.parameters:
                        self.logger.debug("[SEOJI] auto_login exist in self.parameters")
                        if 'password' in self.parameters['nexell_ext']:
                            self.logger.debug("[SEOJI] password in self.parameters['nexell_ext']")
                            nexell_param = self.parameters
                            nexell_cmd = ['echo', nexell_param['nexell_ext']['password'], '>', nexell_param['nexell_ext']['device_path']]
                            for i in range(5):
                                self.logger.debug("[SEOJI] nexell_cmd: " + str(nexell_cmd))
                                command_output = self.run_command(nexell_cmd)
                else:
                    self.logger.debug("[SEOJI] sendline '%s/bin/lava-test-runner %s/%s'" % (lava_test_results_dir, lava_test_results_dir, running))
                    test_connection.sendline(
                ...
                while self._keep_running(test_connection, test_connection.timeout, connection.check_char):
                    ...
                    if feedbacks and time.time() - last_check > test_connection.timeout:
                        for feedback in feedbacks:
                            self.logger.debug("[SEOJI] feedback:" + str(feedback))
    ...
    def _keep_running(self, test_connection, timeout, check_char):
        ...
        self.logger.debug("[SEOJI] self.patterns.values(): " + str(self.patterns.values()))
        self.logger.debug("[SEOJI] self.patterns.keys(): " + str(self.patterns.keys()))
