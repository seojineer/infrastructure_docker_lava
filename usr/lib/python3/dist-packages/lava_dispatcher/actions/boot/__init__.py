    ...
    def populate(self, parameters):
        ...
        if self.has_prompts(parameters):
            ...
            if self.test_has_shell(parameters):
                ...
                self.logger.debug("[SEOJI] boot/__init__.py parameters: %s", parameters)
...
class AutoLoginAction(Action):
    ...
    def validate(self):  # pylint: disable=too-many-branches
        ...
        self.logger.debug("[SEOJI] Autologin parameters: " + str(self.parameters))
        if params:
    ...
    def check_kernel_messages(self, connection, max_end_time, fail_msg):
        ...
        if 'command_param3' in params:
            nexell_cmd = [params['command'], params['command_param3'], params['dir_name'], params['device_path']]
        ...
        if self.booting:
            ...
            if 'username' in params:
                if params['username'] == 'nexell':
                    self.logger.debug("[SEOJI] pass checking kernel-start-message.")
                    ...
                    self.logger.debug("[SEOJI] self.params: " + str(self.params))
                    self.logger.debug("[SEOJI] self.parameters: " + str(self.parameters))
...
class OverlayUnpack(Action):
    ...
    def validate(self):
        super().validate()
        self.logger.debug("[SEOJI] OverlayUnpack() validate()")
