...
class ReadFeedback(Action):
    ...
    def run(self, connection, max_end_time):
        self.logger.debug("[SEOJI] ReadFeedback()")
        feedbacks = []
...
class FinalizeAction(Action):
    ...
    def run(self, connection, max_end_time):
        ...
        for protocol in self.job.protocols:
            self.logger.debug("[SEOJI] power.py run()")
